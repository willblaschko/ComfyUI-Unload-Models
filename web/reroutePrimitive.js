	...
	init() {
		...
		LGraph.prototype.configure = function () {
			...
			for (const n of app.graph._nodes) {
				...
				if (REROUTE_UNLOAD.includes(n.type)) {
					n.onGraphConfigured();
				}
			}
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		function addOutputHandler() {
			...
			nodeType.prototype.getFirstReroutedOutput = function (slot) {
				...
				for (const l of links) {
					...
					if (!REROUTE_UNLOAD.includes(node.type)) {
						return { node, link };
					}
		...
		if (REROUTE_UNLOAD.includes(nodeData.name)) {
			...
			nodeType.prototype.onAdded = function () {
				...
				this.inputs[0].label = "Any";
				this.outputs[0].label = "Any";
			};
			...
			nodeType.prototype.getReroutedInputs = function (slot) {
				let nodes = [{ node: this }];
				let node = this;
				while (REROUTE_UNLOAD.includes(node?.type)) {
			...
			nodeType.prototype.onConnectionsChange = function (type, _, connected, link_info) {
				...
				if (REROUTE_UNLOAD.includes(rootInput.node.type)) {
			...
			nodeType.prototype.computeSize = function () {
				...
				if (this.flags?.collapsed) {
					return [75, 25];  // Ensuring a minimum width when collapsed
				} else if (this.widgets?.length) {
					return r;
				} else {
					let minWidth = 150;  // Set a minimum width for the node
					if (this.outputs?.[0]?.label) {
						const textWidth = LiteGraph.NODE_TEXT_SIZE * this.outputs[0].label.length * 0.6 + 30;
